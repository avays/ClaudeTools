...
The greps and rule walks below catch **known** failure shapes. They are necessary, not sufficient. Before declaring an audit clean, apply the seven audit dimensions in `.claude/agents/auditor.md` "Audit Principles — beyond pattern-matching":
...
A finding surfaced by these dimensions outranks one surfaced only by grep. When one of them turns up a new failure shape, codify a greppable proxy in `.claude/rules/` so the next audit catches it cheaply.
...
Then `Read` `.ai/specs/{name}.md` end-to-end. Extract: all files in the file tree, all DB tables + columns, all API endpoints (method + path), all test scenarios from "What Is Testable".
...
```bash
gh pr list --repo Digital-Synchrony/ORM --state merged --limit 10 --json number,title,files
gh api "repos/Digital-Synchrony/ORM/pulls/<n>/comments" --jq '.[] | {path, body: (.body | .[:200])}'
```
...
Cross-reference against context files (`.ai/context/SCHEMA.md`, `API_ENDPOINTS.md`, `DOMAINS.md`, `INFRASTRUCTURE.md`, `SHARED_TYPES.md`) — any divergence is a HIGH finding ("spec/context out of sync with code").

### 5. Pass 2 — Pattern-level pitfalls (walk `.claude/rules/`)

Every rule in `.claude/rules/` whose scope matches a file in the diff must be walked. **The rules files are the pattern catalog — no sampling.** Rules by scope:
...
- **Shared / testing** — `shared-types.md`, `testing.md` (including the "Root cause over band-aid" section — any band-aid signature in the diff is a finding; see `.claude/agents/auditor.md` dimension 6 for the grep suite and severity rubric)
...
For every frontend PR, run these greps. Each maps to a rule in `.claude/rules/frontend-components.md` "Recurring correctness rules". Each hit needs triage — open the file, decide if the context matches the rule.

#### Frontend grep recipes

```bash
# 1. Icon-only buttons missing aria-label
rg -n --multiline '<button[^>]*\btitle="[^"]*"[^>]*>' packages/ui/src packages/frontend/src \
  | rg -v 'aria-label='

# 2. Inputs whose only label is placeholder
rg -n --multiline '<input[^>]*\bplaceholder="[^"]*"[^>]*>' packages/ui/src packages/frontend/src \
  | rg -v 'aria-label=|id="'

# 3. console.warn / console.error in render bodies
rg -n 'if\s*\(\s*process\.env\.NODE_ENV' packages/ui/src packages/frontend/src -A 4 \
  | rg 'console\.(warn|error)'

# 4. <Button> without explicit type attribute (default type="submit" inside <form>)
rg -n --multiline '<Button[^>]*>' packages/ui/src/components packages/frontend/src \
  | rg -v 'type="(button|submit|reset)"'

# 8. colSpan derived from a toggleable count
rg -n 'colSpan=\{[^}]+\}' packages/ui/src packages/frontend/src

# 9. Mutations missing onError
rg -n '\.mutate\(' packages/frontend/src -A 6 \
  | rg -B 0 -A 5 '\.mutate\(' | rg -B 5 '^\s*\)\s*;' | rg 'onError'

# 10. Delete paths regex-matching err.message
rg -n '(err|error)\.message\.(match|includes|startsWith)' packages/frontend/src

# 11. Destructive confirmations without ref-latch re-entrancy guard
rg -n 'variant="danger"' packages/frontend/src -A 8 \
  | rg -B 3 '\.mutate\(' | rg 'inFlightRef' || echo "potential missing ref latch"

# 12. Raw <table> in admin pages (also enforced by arch test)
rg -n '<table[\s>]' packages/frontend/src/pages/Admin*.tsx

# 13. i18next || fallback antipattern
rg -n "t\(['\"][^'\"]+['\"]\)\s*\|\|" packages/ui/src packages/frontend/src
...
rg -n '\.trim\(\)' packages/frontend/src -B 5 | rg 'if \(!' | head

# 17. Localized-label search via keys instead of accessorFn
rg -n --multiline 'search=\{\{[^}]*\bkeys:[^}]*\blabel\b' packages/frontend/src/pages/Admin*.tsx

# 18. Routed editor — static /new alongside :id
rg -nE "path:\s*'[^']+/new'" packages/frontend/src/app/routes.tsx -A 3 \
  | rg -B 1 "path:\s*'[^']+/:id'"

# 19. Backend pageSize caps — callers passing > 100
rg -nE 'use[A-Z][A-Za-z]+\(\s*\d+\s*,\s*(1[5-9]\d|[2-9]\d\d|\d{4,})' packages/frontend/src

# 20. apiName helpers prefixing with _ instead of a letter
rg -nE "function\s+labelToApiName|replace\(/\^\(\\\\d\)/" packages/frontend/src

# 23. Single-entity hooks over list-then-find
rg -nE '\.find\(\(\w+\)\s*=>\s*\w+\.id\s*===' packages/frontend/src/pages/Admin*EditorPage.tsx

# 24. Tailwind important modifier — suffix form (repo convention is prefix)
rg -nE '\b[a-z]+-[0-9.]+![\s"]' packages/ui/src packages/frontend/src

# 25. Internal admin navigation as <a href> instead of <Link>
rg -n '<a\s+[^>]*href=\{?["`]/admin' packages/frontend/src
```
...
```
ApiClient already prefixes `/api/v1`. Any match in new/modified files is a production 404. See `.claude/rules/frontend.md`.

**2. Record-access-check bypass on polymorphic routes**
For every route/service method that accepts `{recordObjectApiName, recordId}`, verify it calls `dataService.getRecord(..., getUserContext(request))` BEFORE the association read/write. Missing check = RBAC/FLS bypass. See `.claude/rules/backend-api.md` "Record Access Checks".

**3. Raw `db` cast to any in JOIN queries**
```bash
grep -rn "(db as any)\|db as any" packages/backend/src/domains/
```
Should be zero hits. JOINs across tenant-scoped tables must use `withTenant(tenantId).transaction(trx => trx.raw...)`. See `.claude/rules/backend-database.md`.

**4. Integration adapters passing inputs via `queryParams`**
```bash
grep -rn "queryParams:" packages/backend/src/domains/**/providers/
```
For typed ops (inputSchema.properties[].in declared), the executor ignores `queryParams`. All fields MUST go in `body`. See `.claude/rules/integration-adapters.md`.

**5. Bespoke timeouts wrapping executor calls**
```bash
grep -rn "Promise.race.*setTimeout\|setTimeout.*reject" packages/backend/src/domains/
```
Executor already has per-request timeout + circuit breaker. Remove the bespoke timer.

**6. Tautological registry count tripwire**
```bash
grep -rnE "EXPECTED_COUNT = \w+\.length" packages/backend/src/domains/**/registry.test.ts
```
...
- Every route path in `.ai/context/API_ENDPOINTS.md` matches the actual `.routes.ts` declaration.
- Every column and index in `.ai/context/SCHEMA.md` matches the migration file.
...
`.claude/agents/auditor.md` "Code-review-style audit checks" for the
full discussion.
...
grep -rn -B2 -A2 '_BYTES\|_BYTE_LIMIT\|_SIZE_LIMIT' packages/*/src --include='*.ts' \
  | grep -B2 -A2 '\.length'
```
...
grep -rn "from '[^']*\.service\.js'" packages/backend/src/domains/llm-composites/ \
  --include='*.routes.ts'

# description length cap (500 chars max for llm-tool composites)
grep -rn "description:" packages/backend/src/domains/llm-composites/ \
...
```bash
grep -rn -B5 -A10 'countQuery\|countAll' packages/backend/src/domains/ \
...
```bash
grep -rn -B3 -A8 "sql<.*>\`" packages/backend/src/domains/ \
...
grep -rnE ': (Criteria|FlowStepConfig|LayoutComponent)( \|| =|>)' \
  packages/frontend/src/components/ packages/ui/src/components/ \
...
grep -rn "example: '{{[^}]*}}'" packages/backend/src/domains/automation \
  packages/backend/src/domains/metadata --include='*.ts'
...
#    (see packages/shared/src/utils/expression-engine.ts:resolvePath)
#  - Actions: record.X | user.{id,name,email} | bare now/today | $NOW/$USER
#    (see packages/backend/src/domains/automation/actions/action-executors.ts
...
contracts based on intuition. See `.claude/agents/auditor.md` dimension
5 ("LLM-facing strings must match runtime, not intuition") for the
audit posture; `.claude/rules/registry.md` "Verify examples against
...
**Agent instructions sync — required after CLAUDE.md, .claude/rules/*, .claude/skills/*, or .claude/agents/* changes:**
...
context-update step but did not re-run `pnpm sync-agents`, breaking the
...
```bash
pnpm --filter @orm/shared build && pnpm --filter @orm/backend typecheck
...
Use the output template in `.claude/agents/auditor.md`:
...
After the audit passes (or after remediation is complete), update the linked GitHub issue's status on the ORM Build project board:
...
- After `/update-context` runs → **Context Complete** (and it stays there:
  **Done** is set only at merge, by the post-merge `/cleanup` step — never
  by `/audit-phase` or `/update-context`; see `.ai/context/PROJECT_BOARD.md`)
